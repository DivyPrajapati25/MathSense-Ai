  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
      ...
      <button onClick={() => setExpanded((p) => !p)}
        className="w-full flex items-start gap-3 sm:gap-4 p-4 sm:p-5 text-left hover:bg-[var(--color-bg-secondary)] transition-colors">
        <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-xs sm:text-sm font-bold flex items-center justify-center shrink-0 mt-0.5">
      ...
      {expanded && (
        <div className="px-4 sm:px-5 pb-4 sm:pb-5 flex flex-col gap-3 sm:gap-4 border-t border-[var(--color-border)]">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4">
            <div className="p-3 sm:p-4 bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)]">
            ...
            <div className="p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
              <p className="text-xs font-semibold text-blue-500 dark:text-blue-400 mb-2 flex items-center gap-1.5 uppercase tracking-wide">
                <Brain className="w-3.5 h-3.5" /> Correct Answer
              </p>
          ...
          {question.teacher_feedback && (
            <div className="p-3 sm:p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-100 dark:border-green-800">
...
// ← "Correct" card removed, now only 3 stats: Obtained, Total, Score
const SummaryStat = ({ label, value, sub, color = "blue" }) => {
  ...
  return (
    <div className={`${palette[color]} rounded-xl p-3 sm:p-4 text-center`}>
...
const StudentAssignmentDetail = () => {
  ...
  if (error || !data) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <button onClick={() => navigate("/student/grading")} className={`${backBtnCls} mb-6`}><ArrowLeft className="w-4 h-4" /> Back to Dashboard</button>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
      <button onClick={() => navigate("/student/grading")} className={backBtnCls}>
        <ArrowLeft className="w-4 h-4" /> Back to Assignments
      </button>

      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5 sm:mb-6">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-[var(--color-text-primary)]">{data.assignment_name}</h1>
          ...
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold self-start shrink-0 ${
        ...
        {/* 3 stats — Correct card removed */}
        <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-5 sm:mb-6">
          <SummaryStat label="Obtained" value={data.obtained_marks} color="blue" />
          <SummaryStat label="Total"    value={data.total_assignment_marks} color="blue" />
      ...
      <div>
        <h3 className="text-lg sm:text-xl font-bold text-[var(--color-text-primary)] mb-4 sm:mb-5 flex items-center gap-2">
